'''Example based on pytorch/examples/mnist'''
...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)
        x = F.relu(x)
        # x = F.dropout2d(x, training=self.training, p=0.5)
        x = self.fc2(x)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, model, device, train_loader, optimizer_, epoch)
